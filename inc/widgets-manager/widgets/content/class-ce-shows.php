 ...
 * @since 1.0.0
 */
class Podcasts_Shows_Carousel extends Widget_Base {
	...
	public function get_name() {
		return 'ce-podcasts-shows-carousel';
	}
	/**
	...
	public function get_title() {
		return __( 'Podcast Shows Carousel', 'cowidgets' );
	}
	/**
	...
	public function get_script_depends() {
		return [ 'ce-podcasts-shows-carousel', 'tiny-slider' ];
    }
	...
	protected function register_podcasts_carousel_controls() {
		\COWIDGETS_Helpers::showsSelectionSection( $this );
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .ce-post-item .entry-titles' => 'color: {{VALUE}}',
				],
			]
		);
	...
	protected function render() {
        ...
        $this->add_render_attribute( 'ce-podcasts-shows-carousel', 'class', [ 'ce-podcasts-shows-carousel', 'ce-post-style-' . $settings['item_style'] ] );
		$this->add_render_attribute( 'ce-podcasts-shows-carousel', 'data-columns', $settings['columns'] );
        $this->add_render_attribute( 'ce-podcasts-shows-carousel', 'data-autoplay', $settings['autoplay'] );
        $this->add_render_attribute( 'ce-podcasts-shows-carousel', 'data-slide-by', ( $settings['slide_by'] == 'page' ? $settings['columns'] : $settings['slide_by'] ) );
        $this->add_render_attribute( 'ce-podcasts-shows-carousel', 'data-loop', $settings['loop'] );
        $this->add_render_attribute( 'ce-podcasts-shows-carousel', 'data-gutter', $settings['gutter']['size'] );
        $this->add_render_attribute( 'ce-podcasts-shows-carousel', 'data-center', $settings['center'] );
		$this->add_render_attribute( 'ce-podcasts-shows-carousel', 'data-mouse-drag', $settings['mouse_drag'] );
		$this->add_render_attribute( 'ce-podcasts-shows-carousel', 'data-edge-padding-side', $settings['edge_padding_side'] );
		$this->add_render_attribute( 'ce-podcasts-shows-carousel', 'data-carousel-controls', $settings['carousel_controls'] );

        $this->add_render_attribute( 'ce-post-item', 'class', ['ce-post-item'] );

        if( $settings['ce_animation'] != 'none' ){
            $this->add_render_attribute( 'ce-posts-item', 'class', ['ce-animation', 'ce-animation--'.$settings['ce_animation'], 'ce-animation-manual' ] );
            $this->add_render_attribute( 'ce-posts-item', 'data-speed', $settings['ce_animation_speed'] );
            
        }

        if( isset( $settings['edge_padding'] ) && !empty( $settings['edge_padding'] ) )
            $this->add_render_attribute( 'ce-podcasts-shows-carousel', 'data-edge-padding', $settings['edge_padding']['size'] );

        if( isset( $settings['start_index'] ) && !empty( $settings['start_index'] ) )
            $this->add_render_attribute( 'ce-podcasts-shows-carousel', 'data-start-index', $settings['start_index'] );
		...
		<?php if( $settings['carousel_controls'] ){ ?>
			<div class="ce-carousel-head">
                <div class="ce-podcasts-shows-carousel-controls">
		...
		<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'ce-podcasts-shows-carousel' ) ); ?>>
            ...
            <?php
				...
				if( !empty( $settings['items_per_page'] ) ){
					$args['number'] = $settings['items_per_page'];
				}
				
                if( is_array( $settings['items_categories'] ) && !empty( $settings['items_categories'] ) ) {
					$args['include'] = array();
					for( $ii = 0; $ii < count( $settings['items_categories'] ); $ii++ ){
						$term_ = get_term_by( 'slug', $settings['items_categories'][$ii], 'podcast_shows' );
                		$args['include'][] = $term_->term_id;
					}                 
                }
								
                $shows = get_terms( $args );				
