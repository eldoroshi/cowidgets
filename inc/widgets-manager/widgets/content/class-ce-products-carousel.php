...
class Products_Carousel extends Widget_Base {
	...
	public function get_title() {
		return __( 'Products Carousel', 'cowidgets' );
	}
	/**
	...
	public function get_icon() {
		return 'ce-icon-products-carousel';
	}
	/**
	...
	public function get_script_depends() {
		return [ 'ce-products-carousel', 'tiny-slider' ];
    }
	...
	protected function register_controls() {
		$this->register_products_carousel_controls();
	}
	/**
	 ...
	 * @access protected
	 */
	protected function register_products_carousel_controls() {
	...
	protected function render() {
        ...
        $this->add_render_attribute( 'ce-products-carousel', 'class', [ 'ce-products-carousel', 'ce-products-style-' . $settings['item_style'] ] );
		$this->add_render_attribute( 'ce-products-carousel', 'data-columns', $settings['columns'] );
        $this->add_render_attribute( 'ce-products-carousel', 'data-autoplay', $settings['autoplay'] );
        $this->add_render_attribute( 'ce-products-carousel', 'data-slide-by', ( $settings['slide_by'] == 'page' ? $settings['columns'] : $settings['slide_by'] ) );
        $this->add_render_attribute( 'ce-products-carousel', 'data-loop', $settings['loop'] );
        $this->add_render_attribute( 'ce-products-carousel', 'data-gutter', $settings['gutter']['size'] );
        $this->add_render_attribute( 'ce-products-carousel', 'data-center', $settings['center'] );
		$this->add_render_attribute( 'ce-products-carousel', 'data-mouse-drag', $settings['mouse_drag'] );
		$this->add_render_attribute( 'ce-products-carousel', 'data-edge-padding-side', $settings['edge_padding_side'] );
		$this->add_render_attribute( 'ce-products-carousel', 'data-carousel-controls', $settings['carousel_controls'] );

        $this->add_render_attribute( 'ce-products-item', 'class', ['ce-product-item'] );

        if( $settings['ce_animation'] != 'none' ){
            $this->add_render_attribute( 'ce-products-item', 'class', ['ce-animation', 'ce-animation--'.$settings['ce_animation'], 'ce-animation-manual' ] );
            $this->add_render_attribute( 'ce-products-item', 'data-speed', $settings['ce_animation_speed'] );
            
        }

        if( isset( $settings['edge_padding'] ) && !empty( $settings['edge_padding'] ) )
            $this->add_render_attribute( 'ce-products-carousel', 'data-edge-padding', $settings['edge_padding']['size'] );

        if( isset( $settings['start_index'] ) && !empty( $settings['start_index'] ) )
            $this->add_render_attribute( 'ce-products-carousel', 'data-start-index', $settings['start_index'] );
		...
		add_filter( 'woocommerce_post_class', function( $classes ){
			$settings = $this->get_settings_for_display();
			$classes[] = 'ce-product-item';
			$classes[] = 'ce-animation';
			$classes[] = 'ce-animation--'.$settings['ce_animation'];
			$classes[] = 'ce-animation-manual';
			return $classes;
		} );

		?>
		
		<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'ce-products-carousel' ) ); ?>>
			...
			<?php if( $settings['carousel_controls'] ): ?>
						...
						<div class="ce-products-carousel-controls">
