( function( $ ) {
	 ...
	 *
	 */
	var WidgetShowsCarousel = function( $scope, $ ) {
        const element = $scope[0].querySelector( '.ce-podcasts-shows-carousel' );
        ...
        if( parseInt( element.dataset.carouselControls ) ){
            ...
            element.closest('.elementor-widget-container').querySelectorAll( '.ce-podcasts-shows-carousel-controls .ce-prev' ).forEach( (el) => el.onclick = function(e){
                e.preventDefault();
                slider.goTo('prev');
            });
            element.closest('.elementor-widget-container').querySelectorAll( '.ce-podcasts-shows-carousel-controls .ce-next' ).forEach( (el) => el.onclick = function(e){
                e.preventDefault();
                slider.goTo('next');
            });
        }
	...
	$( window ).on( 'elementor/frontend/init', function () {
		...
		elementorFrontend.hooks.addAction( 'frontend/element_ready/ce-podcasts-shows-carousel.default', WidgetShowsCarousel );
