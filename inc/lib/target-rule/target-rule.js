;(function ( $, window, undefined ) {
			...
			ajax: {
			    ...
			    data: function (params) {
			      	return {
				        ...
				        page: params.page,
				        action: 'codeless_get_posts_by_query'
			    	};
				},
	...
	var update_target_rule_input = function(wrapper) {
		...
		wrapper.find('.codeless-target-rule-condition').each(function(i) {
	...
	var update_close_button = function(wrapper) {
		...
		rules 		= wrapper.find('.codeless-target-rule-condition');
		show_close	= false;
	...
	var update_exclusion_button = function( force_show, force_hide ) {
		...
		var exclude_conditions = exclude_on.find('.codeless-target-rule-condition');
	...
	$(document).ready(function($) {
		...
		jQuery( '.codeless-target-rule-condition' ).each( function() {
			var $this 			= $( this ),
		...
		jQuery( document ).on( 'change', '.codeless-target-rule-condition select.target_rule-condition' , function( e ) {
			...
			if( 'specifics' == this_val ) {
				$this.closest( '.codeless-target-rule-condition' ).next( '.target_rule-specific-page-wrap' ).slideDown( 300 );
			} else {
				$this.closest( '.codeless-target-rule-condition' ).next( '.target_rule-specific-page-wrap' ).slideUp( 300 );
			}
		...
		jQuery( '.ast-target-rule-selector-wrapper' ).on( 'click', '.target_rule-add-rule-wrap a', function(e) {
			...
			var $this 	= jQuery( this ),
				...
				template  = wp.template( 'codeless-target-rule-' + type + '-condition' ),
				field_wrap 		= $this.closest('.ast-target-rule-wrapper');
		...
		jQuery( '.ast-target-rule-selector-wrapper' ).on( 'click', '.target_rule-condition-delete', function(e) {
			var $this 			= jQuery( this ),
				rule_condition 	= $this.closest('.codeless-target-rule-condition'),
				field_wrap 		= $this.closest('.ast-target-rule-wrapper');
				cnt 			= 0,
			...
			if ( 'exclude' == data_type ) {
				...
				if ( 1 === field_wrap.find('.target_rule-condition').length ) {
					...
					$this.parent('.codeless-target-rule-condition').next('.target_rule-specific-page-wrap').remove();
			...
			field_wrap.find('.codeless-target-rule-condition').each(function(i) {
