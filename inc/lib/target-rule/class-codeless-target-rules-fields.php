<?php
/**
 * Codeless Advanced Headers Bar Post Meta Box
 *
 * @package   Codeless Pro
 */

namespace COWIDGETS\Lib;

/**
 * Meta Boxes setup
 */
class Codeless_Target_Rules_Fields {
	...
	public function __construct() {
		define( 'CODELESS_TARGET_RULE_URI', plugins_url( '/', __FILE__ ) );
		define( 'CODELESS_TARGET_RULE_VER', '1.0.0' );
		define( 'CODELESS_TARGET_RULE_DIR', plugin_dir_path( __FILE__ ) );

		add_action( 'admin_action_edit', array( $this, 'initialize_options' ) );
		add_action( 'wp_ajax_codeless_get_posts_by_query', array( $this, 'codeless_get_posts_by_query' ) );
	}
	...
	public static function get_location_selections() {
		...
		$post_types = apply_filters( 'codeless_location_rule_post_types', array_merge( $post_types, $custom_post_type ) );

		$special_pages = array(
			'special-404'    => __( '404 Page', 'cowidgets' ),
			'special-search' => __( 'Search Page', 'cowidgets' ),
			'special-blog'   => __( 'Blog / Posts Page', 'cowidgets' ),
			'special-front'  => __( 'Front Page', 'cowidgets' ),
			'special-date'   => __( 'Date Archive', 'cowidgets' ),
			'special-author' => __( 'Author Archive', 'cowidgets' ),
		);

		if ( class_exists( 'WooCommerce' ) ) {
			$special_pages['special-woo-shop'] = __( 'WooCommerce Shop Page', 'cowidgets' );
		}

		$selection_options = array(
			'basic'         => array(
				'label' => __( 'Basic', 'cowidgets' ),
				'value' => array(
					'basic-global'    => __( 'Entire Website', 'cowidgets' ),
					'basic-singulars' => __( 'All Singulars', 'cowidgets' ),
					'basic-archives'  => __( 'All Archives', 'cowidgets' ),
				),
			),

			'special-pages' => array(
				'label' => __( 'Special Pages', 'cowidgets' ),
				'value' => $special_pages,
			),
		);
		...
		$selection_options['specific-target'] = array(
			'label' => __( 'Specific Target', 'cowidgets' ),
			'value' => array(
				'specifics' => __( 'Specific Pages / Posts / Taxanomies, etc.', 'cowidgets' ),
			),
		);
		 ...
		 * @since 1.0.0
		 */
		return apply_filters( 'codeless_display_on_list', $selection_options );
	}
	...
	public static function get_user_selections() {
		$selection_options = array(
			'basic'    => array(
				...
				'value' => array(
					...
					'logged-in'  => __( 'Logged In', 'cowidgets' ),
					'logged-out' => __( 'Logged Out', 'cowidgets' ),
				),
			),

			'advanced' => array(
				'label' => __( 'Advanced', 'cowidgets' ),
				'value' => array(),
			),
		);
		 ...
		 * @since 1.0.0
		 */
		return apply_filters( 'codeless_user_roles_list', $selection_options );
	}
	 ...
	 * @since  1.0.0
	 */
	function codeless_get_posts_by_query() {
	 ...
	 * This is only run from codeless_get_posts_by_query()
	 *
	...
	public function admin_styles() {
		wp_enqueue_script( 'codeless-select2', CODELESS_TARGET_RULE_URI . 'select2.js', array( 'jquery' ), CODELESS_TARGET_RULE_VER, true );
		...
		if ( '' !== $wp_lang ) {
			...
			if ( isset( $select2_lang[ $wp_lang ] ) && file_exists( CODELESS_TARGET_RULE_DIR . 'i18n/' . $select2_lang[ $wp_lang ] . '.js' ) ) {
				$ast_lang = $select2_lang[ $wp_lang ];
				wp_enqueue_script(
					'codeless-select2-lang',
					CODELESS_TARGET_RULE_URI . 'i18n/' . $select2_lang[ $wp_lang ] . '.js',
					array(
						'jquery',
						'codeless-select2',
					),
					CODELESS_TARGET_RULE_VER,
					true
				);
			}
		}
		...
		wp_register_script(
			'codeless-target-rule',
			CODELESS_TARGET_RULE_URI . 'target-rule.js',
			array(
				'jquery',
				'codeless-select2',
			),
		...
		wp_enqueue_script( 'codeless-target-rule' );

		wp_register_script(
			'codeless-user-role',
			CODELESS_TARGET_RULE_URI . 'user-role.js',
			array(
				'jquery',
			),
			CODELESS_TARGET_RULE_VER,
		...
		wp_enqueue_script( 'codeless-user-role' );

		wp_register_style( 'codeless-select2', CODELESS_TARGET_RULE_URI . 'select2.css', '', CODELESS_TARGET_RULE_VER );
		wp_enqueue_style( 'codeless-select2' );
		wp_register_style( 'codeless-target-rule', CODELESS_TARGET_RULE_URI . 'target-rule.css', '', CODELESS_TARGET_RULE_VER );
		wp_enqueue_style( 'codeless-target-rule' );
		...
		$localize_vars = array(
			'ast_lang'      => $ast_lang,
			'please_enter'  => __( 'Please enter', 'cowidgets' ),
			'please_delete' => __( 'Please delete', 'cowidgets' ),
			'more_char'     => __( 'or more characters', 'cowidgets' ),
			'character'     => __( 'character', 'cowidgets' ),
			'loading'       => __( 'Loading more results…', 'cowidgets' ),
			'only_select'   => __( 'You can only select', 'cowidgets' ),
			'item'          => __( 'item', 'cowidgets' ),
			'char_s'        => __( 's', 'cowidgets' ),
			'no_result'     => __( 'No results found', 'cowidgets' ),
			'searching'     => __( 'Searching…', 'cowidgets' ),
			'not_loader'    => __( 'The results could not be loaded.', 'cowidgets' ),
			'search'        => __( 'Search pages / post / categories', 'cowidgets' ),
		);
		wp_localize_script( 'codeless-select2', 'astRules', $localize_vars );
	}
	...
	public static function target_rule_settings_field( $name, $settings, $value ) {
		...
		$add_rule_label = isset( $settings['add_rule_label'] ) ? $settings['add_rule_label'] : __( 'Add Rule', 'cowidgets' );
		$saved_values   = $value;
		$output         = '';
		...
		/* WP Template Format */
		$output .= '<script type="text/html" id="tmpl-codeless-target-rule-' . $rule_type . '-condition">';
		$output .= '<div class="codeless-target-rule-condition ast-target-rule-{{data.id}}" data-rule="{{data.id}}" >';
		...
		$output .= '</div> <!-- codeless-target-rule-condition -->';
	...
	public static function get_post_target_rule_options( $post_type, $taxonomy ) {
		...
		$all_posts                          = sprintf( __( 'All %s', 'cowidgets' ), $post_label );
		$post_option[ $post_name . '|all' ] = $all_posts;

		if ( 'pages' != $post_key ) {
			/* translators: %s post label */
			$all_archive                                = sprintf( __( 'All %s Archive', 'cowidgets' ), $post_label );
		...
		if ( in_array( $post_type->name, $taxonomy->object_type ) ) {
			...
			$tax_archive = sprintf( __( 'All %s Archive', 'cowidgets' ), $tax_label );
	...
	public static function generate_target_rule_selector( $type, $selection_options, $input_name, $saved_values, $add_rule_label ) {
		...
		foreach ( $saved_values['rule'] as $index => $data ) {
			$output .= '<div class="codeless-target-rule-condition ast-target-rule-' . $index . '" data-rule="' . $index . '" >';
			/* Condition Selection */
			$output .= '<span class="target_rule-condition-delete dashicons dashicons-dismiss"></span>';
			$output .= '<div class="target_rule-condition-wrap" >';
	...
	public function get_current_layout( $layout_id, $option ) {
		...
		$current_layout = apply_filters( 'codeless_target_page_settings', $current_layout, $layout_id );
	...
	public static function target_user_role_settings_field( $name, $settings, $value ) {
		...
		/* WP Template Format */
		$output         .= '<script type="text/html" id="tmpl-codeless-user-role-condition">';
			$output     .= '<div class="codeless-user-role-condition ast-user-role-{{data.id}}" data-rule="{{data.id}}" >';
		...
		foreach ( $selection_options as $group => $group_data ) {
			...
			$output .= '</optgroup>';
					...
					$output .= '</select>';
				$output     .= '</div>';
			$output         .= '</div> <!-- codeless-user-role-condition -->';
		$output             .= '</script>';
		...
		foreach ( $saved_values as $index => $data ) {
			$output     .= '<div class="codeless-user-role-condition ast-user-role-' . $index . '" data-rule="' . $index . '" >';
				$output .= '<span class="user_role-condition-delete dashicons dashicons-dismiss"></span>';
				/* Condition Selection */
				$output     .= '<div class="user_role-condition-wrap" >';
	...
	public function get_posts_by_conditions( $post_type, $option ) {
		...
		if ( is_array( self::$current_page_data ) && isset( self::$current_page_data[ $post_type ] ) ) {
			return apply_filters( 'codeless_get_display_posts_by_conditions', self::$current_page_data[ $post_type ], $post_type );
		}
		...
		if ( ! empty( $already_set_rule ) ) {
			add_action(
				...
				function() use ( $already_set_rule ) {
					...
					$notice = sprintf( __( 'The same display setting is already exist in %s post/s.', 'cowidgets' ), $rule_set_titles );
 ...
 */
Codeless_Target_Rules_Fields::get_instance();
